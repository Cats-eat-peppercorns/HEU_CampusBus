...
import com.example.myapplication.databinding.FragmentDriverLoginBinding;
public class DriverLoginFragment extends Fragment {
    private FragmentDriverLoginBinding binding;
    private LoginActivity loginActivity;
    public DriverLoginFragment(LoginActivity loginActivity) {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding=FragmentDriverLoginBinding.inflate(getLayoutInflater());
    ...
    public void loginToMainActivity() {
        binding.loginBtn.setOnClickListener(view -> {
            if(binding.accountEt.getText().length() == 0&&binding.passwordEt.getText().length() == 0) //如果内容为空则可以快捷登录
            {
                SharedPreferences sharedPreferences=requireActivity().getSharedPreferences("user", Context.MODE_PRIVATE);
                binding.accountEt.setText(sharedPreferences.getString("driver_account",""));
                binding.passwordEt.setText(sharedPreferences.getString("driver_password",""));
            }
            ...
            } else if (binding.accountEt.getText().length() != 0 && binding.passwordEt.getText().length() == 0)//密码输入为空
            {
                ...
                loginActivity.driverLoginRequest(accountLogin, passwordLogin);
            }
        });
    }
    public void register() {
        binding.registerGoBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(requireContext(), DriverRegisterActivity.class));
            }
        });
    }
}